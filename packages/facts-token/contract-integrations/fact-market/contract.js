...
function roundDown(v) {
  return new bignumber_default(v)
    .integerValue(bignumber_default.ROUND_DOWN)
    .toNumber();
}
function roundUp(v) {
  return new bignumber_default(v)
    .integerValue(bignumber_default.ROUND_CEIL)
    .toNumber();
}
...
function buy({ contract, contracts, transaction }) {
  return async (state, action) =>
    src_default
      ...
      .map(getPriceAndFee)
      .chain(({ price, fee }) =>
        fromPromise2(registerInteraction)({
          price,
          fee,
          contracts,
          tx: transaction.id,
          allowTx: action.input.tx,
          contract: contract.id,
        })
      )
      .chain(({ price, fee }) =>
        fromPromise2(claimPair)({
          price,
          fee,
          pair: state.pair,
          tx: action.input.tx,
...
var creatorCut = ({ type, price, fee, state, action }) => {
  if (type === 'ok') {
    if (action.input.position === state.position) {
...
// src/write/sell.js
var { fromPromise: fromPromise3 } = src_default;
function sell({ contracts }) {
  return async (state, action) => {
    return src_default
      .of({ state, action })
      .map(validate2)
      ...
      .chain((amount) =>
        fromPromise3(contracts.write)(state.pair, {
          function: 'transfer',
      ...
      .chain(() => fromPromise3(creatorDistribute)({ state, contracts }))
      .map((input) => {
...
var calcualateSellAmount = ({ state, action }) => {
  const balances = getBalances({ state, action });
  return of2(balances)
    .chain((balances2) =>
      ce(
        (balances2[action?.caller] || 0) < roundDown(action?.input?.qty),
...
var calculateSell = (supply, qty) =>
  roundUp(calculatePrice(1, 1, supply, supply - roundDown(qty))) * -1;
var subtractBalance = ({ state, action, type }) => {
  if (type === 'ok') {
    if (action.input.position === 'support') {
      state.balances[action.caller] =
...
var creatorDistribute = async ({ state, contracts }) => {
  if (state.creator_cut > 49e3) {
