...
export function buy({ contract, contracts }) {
  return async (state, action) => {
    return Async.of({ state, action })
      ...
      .bichain(
        () => fromPromise(rejectU)({ state, action, contracts }),
        () => fromPromise(claimU)({ state, action, contracts })
      )
      .chain((previous) =>
        fromPromise(buyFactMarket)({ action, previous, contracts })
      )
 ...
 * @return {*}
 */
function validateTransfer({ state, action, claimable, contract }) {
  ...
  if (contract.id !== claim.to) return Rejected({ state, action });
